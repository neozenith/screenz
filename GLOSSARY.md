...
| dry run | A run that resolves rules into a plan and prints it without moving any window (`--dry-run`, `-n`). The plan is computed identically whether or not it is applied. |
...
| group | Every window of one application, keyed by bundle id; the unit `status` reports over. Selectors, claiming and placement all act on individual windows. |
...
| read-back | The frame re-read from the window server after a placement and compared against the target within tolerance. AX set-frame is a proposal apps clamp silently, so the read-back is the only honest success signal; only demo mode simulates it (ADR-0018). |
...
| result | One action's verdict: `ok` (read-back within tolerance), `clamped` (outside it) or `error` (the placement call failed). Anything but `ok` fails the run. |
...
| usable frame | The display area regions are computed over: the visible frame with the menu bar strip carved out. Carried in code as `Display.VisibleFrame` (`visible_frame` in JSON). |
| verb | A command screenz dispatches on: `doctor`, `status`, `apply`, `list`, `init`, `update` and `version`. Each also answers to its initial (ADR-0024). A profile is never a verb; it is named by flag (ADR-0025). |
