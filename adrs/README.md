...
Decisions are immutable in substance; a change of mind is a new ADR that supersedes the old one with links both ways, and supersession may retire a single clause (recorded in the earlier record's Status and/or Relates to rows, never by rewriting the clause).
...
| [0006](0006-report-unactionable-windows.md) | ADR2.2 | Never hide what the tool cannot act on; incomplete enumeration fails the run |
