...
- **Because**: everything upstream of placement - discovery, matching, the plan and every target frame - is then genuinely computed, and demos render anywhere. Simulated placement reports every frame as landed, so the ACTUAL and RESULT columns and the exit code are fabricated, not measured
