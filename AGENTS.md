...
- Check `adrs/index.md` before asking about or revisiting a design choice; self-answer from existing decisions, and record new binding decisions as a new ADR (never rewrite an accepted one).
- **Records are authored as YAML.** `adrs/NNNN-slug.yml` is the source; the matching `.md`, plus `index.md`, `graph.md`, `graph.json` and `graph.html`, are generated by `make adrs`.
  Never hand-edit a generated file — the edit is lost on the next render.
